	...
	for (int i = 0; i < n_colsA; i++) {
		...
		for (int j = i + 1; j < n_rowsA; j++) {
			...
			for (int k = j; k < n_colsA; k++) {
				...
				if (col_ind[row_ptr_begin[j + 1] + k ] != col_ind[row_ptr_begin[j] + k]) {
					...
					row_ptr_endU[j] = row_ptr_beginU[j] + (row_ptr_end[j] - row_ptr_beginU[j] - k);
					if (cdiff < 0) {
						...
						if (row_ptr_begin[j] + k < row_ptr_end[j]) {
							...
							for (int cd = 0; cd < abs(cdiff); cd++) {
								valuesU[row_ptr_begin[j] + k + cd] = values[row_ptr_begin[j] + k + cd];
								col_indU[row_ptr_begin[j] + k + cd] = col_ind[row_ptr_begin[j] + k + cd];
								row_ptr_endU[j] = row_ptr_endU[j] + 1;
							}
						}
					}
					...
					while (col_ind[row_ptr_begin[j] + k + skipu] != k) {
						skipu++;
					}
					valuesU[row_ptr_beginU[j] + offset - skipK] = values[row_ptr_begin[j] + k + skipu] - mult * values[row_ptr_begin[i] + k + skipu];
